# 👋 Hi, I'm Nabeel Ahmed Ansari

🚀 **AI Engineer | Machine Learning | LLMs | Computer Vision | RAG Systems**

AI Engineer with a Master’s in Computer Science (The City College of New York), specializing in building production-ready AI systems across LLMs, deep learning, and intelligent automation.
✔ Built RAG pipelines with millisecond-level response times
✔ Developed 90%+ accurate medical imaging models
✔ Delivered AI applications used by real users
...
`Python` `C++` `TypeScript` `SQL` `Shell Scripting`

### 🤖 AI / ML

`PyTorch` `TensorFlow` `Scikit-learn` `Transformers` `LangChain` `LangGraph` `RAG` `FAISS`

### 🌐 Backend & Web

`Django` `Flask` `Node.js` `Streamlit`

### ☁️ Cloud & DevOps

`AWS (SageMaker, EC2, S3, Lambda)` `Docker` `Kubernetes` `Git`
